/*
  cap19_integrMC.c: Computing the error in the MC integration.
...
int main(int argc, char *argv[]) {
  int i, j, d, nPoints, meas;
  double L, fact, tmp, sum, sum2;

  if (argc != 5) {
    fprintf(stderr, "usage: %s <d> <L> <nPoints> <seed>\n", argv[0]);
    exit(1);
  }
  
  d = (int)atoi(argv[1]);
  L = (double)atof(argv[2]);
  nPoints = (int)atoi(argv[3]);
  ...
  for (i = 1; i <= nPoints; i++) {
    tmp = 0.;
    for (j = 0; j < d; j++) tmp += FRANDOM;
    sum += exp(-L * tmp);
    sum2 += exp(-2. * L * tmp);
    if (i == meas) {
      printf("%d %.12f %.12f\n", i, fact * sum / i,
	     fact * sqrt(sum2-sum*sum/i) / i);
      meas *= 2;
    }
  }
  return 0;

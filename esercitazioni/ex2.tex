...
nell'{\em header} del programma, e chiamare il compilatore \texttt{gcc} con l'opzione -lm, cio\`e:\\
  \texttt{gcc matematica.c -lm -o matematica.x}
...
\item Eseguire il programma \texttt{circle.c} un numero di volte sufficiente ad avere una distribuzione di punti su tutta la traiettoria circolare; salvare
  ...
  {\em Ricordatevi che se volete inserire un'intestazione in un file di dati letto da gnuplot, dovete farla precedere dal carattere cancelletto (\texttt{\#})}.
\item Utilizzare \texttt{gnuplot} per graficare la traiettoria, plottando $y$ in funzione di $x$.
  {\em Per plottare solo due colonne di un file che ne contiene molte, in \texttt{gnuplot} si usa l'opzione \texttt{using (u)}. Nel nostro caso, se volessimo plottare solo la seconda e la terza colonna scriveremmo:
    \\
    \texttt{plot 'traiettoria.dat' u 2:3}}
\item 
...
\item Salvare i tre grafici in tre file separati, chiamati: \texttt{traiettoria.gif; x.gif; y.gif}.
  {\em Per salvare i grafici con gnuplot, bisogna selezionare un opportuno terminale con il comando
    \texttt{set term <nome\_terminale>} e riplottare il grafico sul nuovo terminale con il comando
    replot. }
  \begin{mdframed}[backgroundcolor=gray!10]
\texttt{set term gif
\\
  set out 'traiettoria.gif'
\\
  replot
\\
  set term X11
...
\hrule
\vspace{2mm}\textbf{$\RHD$ Quarta parte (facoltativa)\\}
...
\item (Avanzato): Usando l'help di gnuplot, disegnare il vettore velocit\`a lungo la traiettoria,
  come una freccia che punta nella direzione corretta.
\end{enumerate}  

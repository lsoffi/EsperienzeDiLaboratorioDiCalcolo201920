...
\item Utilizzare \texttt{python} per graficare la traiettoria, plottando $y$ in funzione di $x$.
  Per plottare solo due colonne di un file che ne contiene molte, la funzione \textbf{\lstinline{loadtext}} va chiamata con il parametro \textbf{\lstinline[language=python]{usecols(i,j)}}, dove \lstinline{i} e \lstinline{j} sono le colonne da plottare ($0$ è la prima colonna). Nel nostro caso, se volessimo plottare solo la seconda e la terza colonna scriveremmo nello script \texttt{python}: \\
\begin{mdframed}[backgroundcolor=gray!10]
\begin{lstlisting}
  np.loadtext('traiettoria.dat', usecols(1:2))
\end{lstlisting}
\end{mdframed}
\item 
